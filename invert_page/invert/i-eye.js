window.addEventListener(
	...
	(function () {
		var iEye = {
			...
			css:
				" html {-webkit-filter: invert(100%)!important;background-color: white!important;}" +
			...
			init: function () {
				...
				if (document.getElementById("i-eye-blocker")) {
					document
						.getElementById("i-eye-blocker")
						.parentNode.removeChild(
							document.getElementById("i-eye-blocker")
						);
				}
				...
				chrome.runtime.onMessage.addListener(
					function (request, sender, sendResponse) {
						switch (request.action) {
							...
							case "exclude-page":
								this.invertColor();
								iEyeStorage.updateObject(
									"exclude",
									this.host,
									request.value || window.location.pathname
								);
								break;
							case "reset":
							...
							default:
								console.log(
									"i-eye: Unhandled event: " + request.action
								);
						}
					}.bind(this)
				);
